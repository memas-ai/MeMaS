...
def test_save_then_search_one_corpus(es_client):
    ...
    assert test_corpus.store_and_index([("doc1", text1, Citation("www.docsource1", "SSSdoc1", ""))])
    assert test_corpus.store_and_index([("doc2", text2, Citation("were.docsource2", "SSSdoc2", ""))])
    assert test_corpus.store_and_index([("doc3", text3, Citation("docsource3.ai", "SSSdoc3", ""))])

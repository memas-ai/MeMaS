...
class BasicCorpus(Corpus) :
    ...
    def store_and_index(self, document: str, citation: Citation) -> bool:
        doc_id = uuid.uuid4()
        doc_entity = DocumentEntity(self.corpus_ID, doc_id, citation.source_name, document)
    ...
    def search(self, clue: str) -> list[tuple[str, Citation]]:
        ...
        for score, doc_entity in temp_res :
            # TODO should the basic corpus use the score?

            document_text = doc_entity.document

            # TODO : Need to fetch Source URI and description if applicable
            citation = Citation("source_URI_MISSING", doc_entity.document_name, self.corpus_name, "no description")
        ...
        for score, doc_entity, start_index, end_index in temp_res2 :
            ...
            # Check that the text recovered from the vectors is full sentences and recovered the rest if needed
...
def normalize_and_combine(doc_results : list, vec_results : list) :
    ...
    print("For Doc we have")
    print(doc_results)

    print("For vec we have ")
    print(vec_results)
